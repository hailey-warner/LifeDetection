...
function plot_alpha_vectors_VLD(policy::AlphaVectorPolicy, pomdp, sample::Int)
	...
	b = range(0, 1, length=200)  # belief in P(life)
...
function plot_decision_tree(
	...
	t = TikzGraphs.plot(
		g,
		TikzGraphs.Layouts.Layered(),
		node_labels,
		edge_styles=edge_colors,
		node_style="draw",
		graph_options="nodes={draw,circle}",
	)
...
function plot_pareto_frontier() end
