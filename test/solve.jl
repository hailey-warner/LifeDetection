...
probability_table = [
    ...
    (
        [:a, :l],
        [(a=1, l=1) => 0.9, (a=2, l=1) => 0.1, (a=1, l=2) => 0.05, (a=2, l=2) => 0.95],
    ),
    (
        [:p, :l],
        [(p=1, l=1) => 0.7, (p=2, l=1) => 0.3, (p=1, l=2) => 0.2, (p=2, l=2) => 0.8],
    ),
    (
        [:c, :l],
        [(c=1, l=1) => 0.75, (c=2, l=1) => 0.25, (c=1, l=2) => 0.3, (c=2, l=2) => 0.7],
    ),
    (
        [:h, :a],
        [(h=1, a=1) => 0.9, (h=2, a=1) => 0.1, (h=1, a=2) => 0.15, (h=2, a=2) => 0.85],
    ),
    (
        [:i, :a],
        [(i=1, a=1) => 0.7, (i=2, a=1) => 0.3, (i=1, a=2) => 0.6, (i=2, a=2) => 0.4],
    ),
    (
        [:i, :p],
        [(i=1, p=1) => 0.8, (i=2, p=1) => 0.2, (i=1, p=2) => 0.9, (i=2, p=2) => 0.1],
    ),
    (
        [:i, :c],
        [(i=1, c=1) => 0.8, (i=2, c=1) => 0.2, (i=1, c=2) => 0.7, (i=2, c=2) => 0.3],
    ),
]

bn = bayes_net(nodes, dependencies, probability_table)
pomdp =
    binaryLifeDetectionPOMDP(inst=NUM_INSTRUMENTS, bn=bn, λ=20, k=SENSOR_COST, discount=0.9)
...
if PARETO_FRONTIER == true
    ...
    for λ in range(1, end_λ)
        pomdp = binaryLifeDetectionPOMDP(
