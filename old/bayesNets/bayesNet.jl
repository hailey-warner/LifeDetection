...
push!(
    bn_cont,
    CategoricalCPD{Categorical}(:aa, [:l], [2], [Categorical(p_no), Categorical(p_yes)]),
)
...
push!(
    bn_cont,
    CategoricalCPD{UnivariateDistribution}(
        :r,
        [:af],
        [2],
        [Distributions.Uniform(-0.5, 0.0), LocationScale(-0.5, 0.5, Beta(2, 1))],
    ),
)
